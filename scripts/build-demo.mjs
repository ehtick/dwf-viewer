...
const distDirName = `dist-v${version.replace(/[^0-9A-Za-z._-]/g, '-')}`;
const out = path.join(root, 'demo-dist');
await rm(out, { recursive: true, force: true });
await mkdir(out, { recursive: true });
await cp(path.join(root, 'dist'), path.join(out, distDirName), { recursive: true });
...
app = app
  .replaceAll(/'\.\.\/dist\/index\.js\?v=[^']*'/g, `'./${distDirName}/index.js'`)
