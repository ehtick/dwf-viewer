...
export type W2dPrimitive =
  | (W2dPrimitiveBase & { type: 'polyline'; points: number[] })
  | (W2dPrimitiveBase & { type: 'polygon'; points: number[] })
  | (W2dPrimitiveBase & { type: 'path'; commands: PathCommand[] })
  | (W2dPrimitiveBase & { type: 'text'; x: number; y: number; text: string; size?: number })
  | (W2dPrimitiveBase & { type: 'rect'; x: number; y: number; width: number; height: number });
...
export interface W3dAnimationData {
  ...
  name: string;
  targetRef?: string;
  keyframes?: Array<{ time: number; matrix?: number[]; visibility?: boolean }>;
}
...
export interface W3dModelData {
  ...
  stats: { meshCount: number; vertexCount: number; triangleCount: number; decodedBytes: number; edgeCount?: number; materialCount?: number; textureCount?: number; pmiCount?: number; nodeCount?: number };
...
export function makeLoadedDocument(base: DwfDocument, pageData: PageData[], zip?: ZipPackage, opc?: OpcPackageView): LoadedDwfDocument {
  return {
    ...base,
