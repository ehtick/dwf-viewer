...
import { makeLoadedDocument, type ImagePageData, type LoadedDwfDocument, type PageData, type UnsupportedPageData, type W2dPrimitive, type W2dTextPageData } from './document.js';
...
interface DescriptorInfo {
  ...
  paperHeight?: number;
  clip?: number[];
  w2dTransform?: { sx: number; sy: number; tx: number; ty: number };
}

export function isProbablyClassicDwf(zip: ZipPackage): boolean {
  return zip.entries.some(e => /\.w2d$/i.test(e.name) || /manifest\.xml$/i.test(e.name) || /descriptor\.xml$/i.test(e.name) || /\.hsf$/i.test(e.name));
}

export async function openClassicDwf(zip: ZipPackage): Promise<LoadedDwfDocument> {
  ...
  let manifestSections: ManifestSection[] = [];
  if (manifestEntry) {
    try {
  ...
  if (manifestSections.length > 0) {
    for (const section of manifestSections) {
      ...
      if (!mainGraphic) continue;

      const descriptorResource = resourceByRole(section, /^descriptor$/i) ?? { path: normalizePath(section.name + '/descriptor.xml') };
      let descriptor: DescriptorInfo = {};
      if (descriptorResource && zip.has(descriptorResource.path)) {
        try { descriptor = parseDescriptorInfo(decodeUtf8(await zip.read(descriptorResource.path)), mainGraphic.path); }
        catch (err) { diagnostics.push(diag('warning', 'DWF_DESCRIPTOR_PARSE_FAILED', `Failed to parse descriptor for ${section.title}: ${String(err)}`, descriptorResource.path)); }
      }

      const pageDiagnostics: Diagnostic[] = [];
      ...
      for (const markupXml of section.resources.filter(r => /markup\s+private/i.test(r.role ?? '') && /\.xml$/i.test(r.path))) {
        if (!zip.has(markupXml.path)) continue;
        try {
...
async function discoverW2dResources(zip: ZipPackage, pageData: PageData[], diagnostics: Diagnostic[]): Promise<void> {
...
async function parseW2dResource(zip: ZipPackage, path: string) {
  const bytes = await zip.read(path);
  if (bytesLookTextual(bytes)) return parseW2dText(bytes, path);
  return parseW2dBinary(bytes, path);
}

function parseManifestSections(xml: string, sourcePath: string): ManifestSection[] {
...
function parseDescriptorInfo(xml: string, mainGraphicPath: string): DescriptorInfo {
  ...
  if (paper) {
    ...
    if (clip) info.clip = parseNumberList(clip);
  }
...
function parseMarkupPrivateXml(xml: string, sourcePath: string, transform?: { sx: number; sy: number; tx: number; ty: number }): { primitives: W2dPrimitive[]; diagnostics: Diagnostic[] } {
  const doc = parseXml(xml, sourcePath);
  const primitives: W2dPrimitive[] = [];
  const toLogical = (x: number, y: number) => transform
    ? { x: (x - transform.tx) / transform.sx, y: (y - transform.ty) / transform.sy }
    : { x, y };
  const pointsAttr = (el: Element): number[] => parseNumberList(attrAny(el, 'Points') ?? attrAny(el, 'points') ?? '');
  ...
  const diagnostics: Diagnostic[] = [];
  if (primitives.length > 0) diagnostics.push(diag('info', 'DWF_MARKUP_XML_PARSED', `Parsed ${primitives.length} markup primitive(s) from markup private XML.`, sourcePath));
  return { primitives, diagnostics };
}

function styleFromMarkup(el: Element): { stroke: string; fill?: string; lineWidth: number } {
  let stroke = '#0000ff';
  ...
  for (const p of props) {
    ...
    if (!enabled || value === undefined) continue;
    if (name === 'LINECOLOR') stroke = signedIntColor(value);
    if (name === 'FILLCOLOR') fill = signedIntColor(value);
    if (name === 'LINEWEIGHT') lineWidth = Math.max(1, Number(value) || lineWidth);
    if (name === 'NOFILL' && /true/i.test(attrAny(p, 'BoolValue') ?? '')) fill = undefined;
  }
  return { stroke, fill, lineWidth };
}

function signedIntColor(value: string): string {
...
function resourceByRole(section: ManifestSection, re: RegExp): ManifestResource | undefined {
...
function chooseBestImage(zip: ZipPackage) {
...
function scoreImage(path: string, size: number): number {
  let s = Math.min(size, 10_000_000) / 1000;
  if (/thumbnail|preview/i.test(path)) s += 5000;
  if (extname(path) === 'png') s += 100;
  return s;
}
