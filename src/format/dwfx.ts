import type { Diagnostic, DwfDocument } from './types.js';
import { diag } from './types.js';
import type { OpcPackageView, OpcRelationship } from './opc.js';
...
import { makeLoadedDocument, type ImagePageData, type LoadedDwfDocument, type PageData, type XpsPageData, type UnsupportedPageData, type W3dPageData } from './document.js';
...
interface DwfxManifestResource {
  path: string;
  role?: string;
  mediaType?: string;
  title?: string;
...
interface DwfxManifestSection {
  type: string;
  name: string;
  title: string;
  resources: DwfxManifestResource[];
}

export async function openDwfx(zip: ZipPackage): Promise<LoadedDwfDocument> {
  ...
  if (pageData.length === 0) {
    ...
    for (const fdseqPath of fdseqs) {
      try {
        ...
        if (docRefs.length === 0) diagnostics.push(diag('warning', 'DWFX_NO_DOCREF', `No DocumentReference elements found in ${fdseqPath}.`, fdseqPath));
...
export function isProbablyDwfx(zip: ZipPackage): boolean {
  return zip.has('[Content_Types].xml') && (zip.has('_rels/.rels') || zip.entries.some(e => /\.(fpage|dwfseq)$/i.test(e.name)));
}

async function collectDwfxManifestPages(opc: OpcPackageView, pageData: PageData[], diagnostics: Diagnostic[]): Promise<DwfxManifestSection[]> {
  const manifestPaths = await findDwfManifestPaths(opc, diagnostics);
  const allSections: DwfxManifestSection[] = [];
  ...
  for (const section of allSections.filter(s => isEPlotSection(s))) {
    ...
    if (!fixedPage || !/\.fpage$/i.test(stripQuery(fixedPage.path))) continue;
    const pagePath = stripQuery(fixedPage.path);
    if (!opc.zip.has(pagePath)) continue;
  ...
  if (pageData.length > 0) return allSections;
...
async function findDwfManifestPaths(opc: OpcPackageView, diagnostics: Diagnostic[]): Promise<string[]> {
  const out = new Set<string>();
  const add = (p: string | undefined) => { if (p) out.add(normalizePath(stripQuery(p))); };

  // Package root relationship -> DWFDocumentSequence.dwfseq -> ManifestReference.
  const dwfseqs = new Set<string>();
  try {
    const rootRels = await opc.getRelationships('');
    for (const rel of rootRels) {
      if (/documentsequence/i.test(rel.type) || /\.dwfseq$/i.test(rel.resolvedTarget)) dwfseqs.add(normalizePath(rel.resolvedTarget));
      if (/document$/i.test(rel.type) && /manifest\.xml$/i.test(rel.resolvedTarget)) add(rel.resolvedTarget);
    }
  } catch (err) {
    diagnostics.push(diag('warning', 'DWFX_DWFSEQ_RELS_FAILED', `Failed to inspect DWFx package relationships: ${String(err)}`));
  }
  for (const e of opc.zip.entries) if (/\.dwfseq$/i.test(e.name)) dwfseqs.add(e.name);

  for (const dwfseqPath of dwfseqs) {
    if (!opc.zip.has(dwfseqPath)) continue;
    try {
      ...
      for (const ref of refs) add(ref.getAttribute('Source') ?? ref.getAttribute('source') ?? undefined);
      const rels = await opc.getRelationships(dwfseqPath).catch(() => [] as OpcRelationship[]);
      for (const rel of rels) if (/document/i.test(rel.type) || /manifest\.xml$/i.test(rel.resolvedTarget)) add(rel.resolvedTarget);
    } catch (err) {
  ...
  for (const e of opc.zip.entries) {
    if (/^dwf\/documents\/.*\/manifest\.xml$/i.test(e.name)) add(e.name);
  }

  return Array.from(out).filter(p => opc.zip.has(p));
}

async function findFixedDocumentSequences(opc: OpcPackageView, diagnostics: Diagnostic[]): Promise<string[]> {
  const out = new Set<string>();
  try {
    const rootRels = await opc.getRelationships('');
    for (const rel of rootRels) {
  ...
  for (const o of opc.overrides) {
    if (/fixeddocumentsequence/i.test(o.contentType) || /\.fdseq$/i.test(o.partName)) out.add(o.partName);
  }
  for (const e of opc.zip.entries) {
    if (/\.fdseq$/i.test(e.name)) out.add(e.name);
  }
  return Array.from(out);
}

async function collectFixedDocumentPages(opc: OpcPackageView, fdocPath: string, pageData: PageData[], diagnostics: Diagnostic[]): Promise<void> {
  let xml: string;
  try { xml = await opc.readText(fdocPath); }
  catch (err) {
  ...
  for (const pageContent of pageContents) {
    ...
    if (relId) {
      ...
      if (rel) pagePath = rel.resolvedTarget;
    }
...
async function readFixedPageSize(opc: OpcPackageView, pagePath: string, diagnostics: Diagnostic[]): Promise<{ width: number; height: number }> {
  try {
...
function parseDwfManifestSections(xml: string, sourcePath: string): DwfxManifestSection[] {
...
function isEPlotSection(section: DwfxManifestSection): boolean {
  return /(^|\.)ePlot$/i.test(section.type) || /com\.autodesk\.dwf\.ePlot$/i.test(section.type);
}

function isEModelSection(section: DwfxManifestSection): boolean {
  return /(^|\.)eModel$/i.test(section.type) || /com\.autodesk\.dwf\.eModel$/i.test(section.type);
}

function chooseSectionPreviewResource(section: DwfxManifestSection, zip: ZipPackage): DwfxManifestResource | undefined {
...
function scorePreview(r: DwfxManifestResource): number {
  let s = r.size ?? 0;
  if (/thumbnail/i.test(r.role ?? '')) s += 5_000_000;
  if (/preview/i.test(r.role ?? '')) s += 4_000_000;
  if (/icon/i.test(r.role ?? '')) s -= 2_000_000;
  if (/texture/i.test(r.role ?? '')) s -= 3_000_000;
  return s;
}
...
function resourceByRole(section: DwfxManifestSection, re: RegExp): DwfxManifestResource | undefined {
...
export function getRelationshipById(rels: OpcRelationship[], id: string): OpcRelationship | undefined {
