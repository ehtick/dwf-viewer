...
export function diag(level: DiagnosticLevel, code: string, message: string, source?: string): Diagnostic {
  return source === undefined ? { level, code, message } : { level, code, message, source };
}

export function actionableDiagnostics(diagnostics: readonly Diagnostic[] | undefined): Diagnostic[] {
