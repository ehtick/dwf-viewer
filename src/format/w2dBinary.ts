...
export interface W2dBinaryParseResult {
  ...
  opcodes: number;
  unsupportedOpcodes: Array<{ offset: number; opcode: number | string; message: string }>;
}
...
export function parseW2dBinary(bytes: Uint8Array, sourcePath: string): W2dBinaryParseResult {
...
class BinaryW2dParser {
  ...
  private readonly primitives: W2dPrimitive[] = [];
  private readonly diagnostics: Diagnostic[] = [];
  private readonly unsupportedOpcodes: Array<{ offset: number; opcode: number | string; message: string }> = [];
  private opcodes = 0;
  private decimalRevision = 600;
  ...
  constructor(private readonly bytes: Uint8Array, private readonly sourcePath: string) {
    this.view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  }

  parse(): W2dBinaryParseResult {
    while (this.pos < this.bytes.length) {
      ...
      try {
        ...
        if (opcode === 0x7b) { // '{', extended binary opcode.
          if (!this.parseExtendedBinary(offset)) break;
          continue;
        }
        if (this.parseSingleByteOpcode(opcode)) continue;
  ...
  private parseSingleByteOpcode(opcode: number): boolean {
    switch (opcode) {
      case 0x00:
        return true;
      case 0x03: { // set color RGBA
        ...
        if (this.state.fillOn) this.state.fill = this.state.stroke;
        return true;
      }
      case 0x06: // set font
        this.readBinaryFont();
  ...
  private parseExtendedAscii(start: number): void {
    const { token, expr } = this.readBalancedExpression(start);
    switch (token) {
      case '(W2D': {
        const m = expr.match(/V(\d+)\.(\d+)/i);
        if (m) this.decimalRevision = Number(m[1]) * 100 + Number(m[2]);
        break;
      }
      case '(Color': {
        const nums = numbers(expr);
        if (nums.length >= 3) this.state.stroke = rgb(nums[0]!, nums[1]!, nums[2]!);
        break;
      }
      case '(LineWeight': {
        const nums = numbers(expr);
        if (nums.length >= 1) this.state.lineWidth = Math.max(0.2, Math.abs(nums[0]!));
        break;
      }
      case '(Font': {
        ...
        if (height) this.state.fontHeight = Math.max(1, Math.abs(Number(height)));
        if (rotation) this.state.fontRotation = Number(rotation) || 0;
        if (name) this.state.fontName = name;
        break;
      }
      case '(EndOfDWF':
  ...
  private parseExtendedBinary(start: number): boolean {
  ...
  private readBinaryFont(): void {
    const fields = this.u16();
    if (fields & FONT_NAME_BIT) this.state.fontName = this.readWhipString() || this.state.fontName;
    if (fields & FONT_CHARSET_BIT) this.u8();
    if (fields & FONT_PITCH_BIT) this.u8();
    if (fields & FONT_FAMILY_BIT) this.u8();
    if (fields & FONT_STYLE_BIT) this.u8();
    if (fields & FONT_HEIGHT_BIT) this.state.fontHeight = Math.max(1, Math.abs(this.i32()));
    if (fields & FONT_ROTATION_BIT) this.state.fontRotation = this.u16();
    if (fields & FONT_WIDTH_SCALE_BIT) this.u16();
    if (fields & FONT_SPACING_BIT) this.u16();
    if (fields & FONT_OBLIQUE_BIT) this.u16();
    if (fields & FONT_FLAGS_BIT) this.i32();
  }

  private skipComplexTextOptions(): void {
    ...
    if (this.decimalRevision >= 600 && this.pos < this.bytes.length) {
      ...
      for (let i = 0; i < count; i++) this.readCount();
    }
  }

  private skipScoringOption(): void {
    const count = Math.max(0, this.readCount() - 1);
    for (let i = 0; i < count; i++) this.readCount();
  }

  private pointSetPrimitive(pts: Point[]): void {
    if (this.state.fillOn) this.polygon(pts, this.state.fill ?? this.state.stroke, this.state.stroke);
    else this.polyline(pts);
  }

  private polytriangles(pts: Point[]): void {
    if (!this.state.visible || pts.length < 3) return;
    for (let i = 0; i + 2 < pts.length; i++) {
      this.polygon([pts[i]!, pts[i + 1]!, pts[i + 2]!], this.state.stroke, undefined);
    }
  }

  private polyline(pts: Point[]): void {
    if (!this.state.visible || pts.length < 2) return;
    this.primitives.push({ type: 'polyline', points: flattenPoints(pts), stroke: this.state.stroke, lineWidth: this.state.lineWidth });
  }

  private polygon(pts: Point[], fill: string | undefined, stroke: string | undefined): void {
    if (!this.state.visible || pts.length < 3) return;
    this.primitives.push({ type: 'polygon', points: flattenPoints(pts), fill, stroke, lineWidth: this.state.lineWidth });
  }

  private ellipsePrimitive(center: Point, major: number, minor: number, start: number, end: number, tilt: number, filled = this.state.fillOn): void {
    const pts = ellipsePoints(center, major, minor, start, end, tilt);
    if (filled) this.polygon(pts, this.state.fill ?? this.state.stroke, this.state.stroke);
    else this.polyline(pts);
  }

  private text(p: Point, text: string): void {
    if (!this.state.visible || !text.trim()) return;
  ...
  private pointSet32Relative(): Point[] {
    const count = this.readCount();
    const pts: Point[] = [];
    for (let i = 0; i < count; i++) pts.push(this.point32Relative());
    return pts;
  }

  private pointSet16Relative(): Point[] {
    const count = this.readCount();
    const pts: Point[] = [];
    for (let i = 0; i < count; i++) pts.push(this.point16Relative());
    return pts;
  }

  private pointSetAscii(): Point[] {
    const count = this.readAsciiInteger();
    const pts: Point[] = [];
    for (let i = 0; i < count; i++) pts.push(this.readAsciiPoint());
    return pts;
  }

  private point32Relative(): Point {
    const dx = this.i32();
    const dy = this.i32();
  ...
  private readAsciiPoint(): Point {
    const x = this.readAsciiInteger();
    if (this.peek() === 0x2c) this.pos++;
    const y = this.readAsciiInteger();
    return { x, y };
  }

  private readAsciiInteger(): number {
    this.eatWhitespace();
  ...
  private readWhipString(): string {
    ...
    if (lead === 0x7b) { // binary UTF-16 string: { int32 length, uint16 data..., }
      ...
      if (this.pos + byteLength > this.bytes.length) throw new Error('Truncated binary WHIP string.');
      const s = new TextDecoder('utf-16le').decode(this.bytes.subarray(this.pos, this.pos + byteLength));
      this.pos += byteLength;
      if (this.peek() === 0x7d) this.pos++;
      return s;
    }
    ...
    while (this.pos < this.bytes.length) {
      ...
      if (c === 0x20 || c === 0x09 || c === 0x0a || c === 0x0d || c === 0x29) break;
      this.pos++;
    }
    return ascii(this.bytes.subarray(start, this.pos));
  }

  private readBalancedExpression(start: number): { token: string; expr: string } {
    this.pos = start;
    let depth = 0;
    let quote = 0;
    let escaped = false;
  ...
  private remaining(): number { return this.bytes.length - this.pos; }
  private peek(): number | undefined { return this.pos < this.bytes.length ? this.bytes[this.pos] : undefined; }
  private u8(): number { if (this.remaining() < 1) throw new Error('Unexpected EOF'); return this.view.getUint8(this.pos++); }
  private i16(): number { if (this.remaining() < 2) throw new Error('Unexpected EOF'); const n = this.view.getInt16(this.pos, true); this.pos += 2; return n; }
  private u16(): number { if (this.remaining() < 2) throw new Error('Unexpected EOF'); const n = this.view.getUint16(this.pos, true); this.pos += 2; return n; }
  private i32(): number { if (this.remaining() < 4) throw new Error('Unexpected EOF'); const n = this.view.getInt32(this.pos, true); this.pos += 4; return n; }
  private u32(): number { if (this.remaining() < 4) throw new Error('Unexpected EOF'); const n = this.view.getUint32(this.pos, true); this.pos += 4; return n; }
}
...
function ellipsePoints(center: Point, major: number, minor: number, start: number, end: number, tilt: number): Point[] {
  let s = start;
  let e = end;
  if (s === 0 && e === 0) e = 0x10000;
  else if (e <= s) e += 0x10000;
  const sweep = e - s;
...
function cleanW2dText(text: string): string {
  return text
    .replace(/\\P/gi, '\n')
    .replace(/\\L|\\l/g, '')
    .replace(/\\O|\\o/g, '')
...
function ascii(bytes: Uint8Array): string { return Array.from(bytes, b => String.fromCharCode(b)).join(''); }
function rgb(r: number, g: number, b: number): string { return `rgb(${clamp8(r)}, ${clamp8(g)}, ${clamp8(b)})`; }
function rgba(r: number, g: number, b: number, a: number): string { return `rgba(${clamp8(r)}, ${clamp8(g)}, ${clamp8(b)}, ${Math.max(0, Math.min(1, a / 255))})`; }
function clamp8(n: number): number { return Math.max(0, Math.min(255, Math.round(n))); }
function numbers(s: string): number[] { return (s.match(/[-+]?(?:\d+\.\d*|\.\d+|\d+)(?:[eE][-+]?\d+)?/g) ?? []).map(Number).filter(Number.isFinite); }
