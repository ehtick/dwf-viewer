 ...
 */
export function createThreeGroupFromW3d(pageOrModel: W3dPageData | W3dModelData, THREE: ThreeLikeNamespace, options: ThreeAdapterOptions = {}): any {
  const model = (pageOrModel as W3dPageData).model ?? pageOrModel as W3dModelData;
  const group = new THREE.Group();
  group.name = model.title ?? 'DWFx W3D Model';
  const materialById = new Map((model.materials ?? []).map(m => [m.id, m] as const));
  const textureById = new Map((model.textures ?? []).map(t => [t.id, t] as const));
  ...
  for (const mesh of model.meshes) {
    ...
    if ((options.doubleSide || srcMaterial?.doubleSided) && THREE.DoubleSide !== undefined) materialOptions.side = THREE.DoubleSide;
    const tex = srcMaterial?.textureId ? textureById.get(srcMaterial.textureId) : undefined;
    if (tex && options.textureResolver) materialOptions.map = options.textureResolver(tex, srcMaterial!);
