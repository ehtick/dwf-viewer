...
import { applyMatrixToContext, IDENTITY, multiplyMatrix, parseBrushColor, parseMatrix, type Matrix2D } from './style.js';
...
export class XpsRenderer {
  ...
  private async renderElementToCanvas(
    ...
    if (name === 'Path' && mode.vectors) {
      ...
      if (path.length > 0) {
        ...
        if (fill) { ctx.fillStyle = fill; ctx.fill(fillRule(el)); }
        if (stroke && thickness > 0) { ctx.strokeStyle = stroke; ctx.lineWidth = thickness; ctx.stroke(); }
        ctx.restore();
        commands++;
      }
  ...
  private renderElementToWasm(el: Element, matrix: Matrix2D, opacity: number, warnings: Diagnostic[]): number {
    ...
    if (name === 'Path') {
      ...
      if (path.length > 0) {
        ...
        if (stroke && thickness > 0) {
          for (const sub of subs) this.wasm.drawPolyline(sub.points, composed, stroke, thickness * composed.a);
        }
        commands++;
      }
    }
