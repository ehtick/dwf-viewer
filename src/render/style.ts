...
export function parseMatrix(input?: string | null): Matrix2D {
  if (!input) return { ...IDENTITY };
  const nums = parseNumberList(input);
  if (nums.length >= 6) return { a: nums[0]!, b: nums[1]!, c: nums[2]!, d: nums[3]!, e: nums[4]!, f: nums[5]! };
  return { ...IDENTITY };
}

export function applyMatrixToContext(ctx: CanvasRenderingContext2D, m: Matrix2D): void {
  ctx.transform(m.a, m.b, m.c, m.d, m.e, m.f);
}

export function parseBrushColor(input?: string | null, opacity = 1): string | undefined {
  if (!input || input === 'Transparent' || input === '{x:Null}') return undefined;
  let s = input.trim();
  if (s.startsWith('{') || s.includes('Brush')) return undefined;
...
export function colorToRgba32(css: string | undefined, fallback = 0xff000000): number {
  ...
  if (s.startsWith('#')) {
    const hex = s.slice(1);
    if (hex.length === 6) return packRgba(parseInt(hex.slice(0, 2), 16), parseInt(hex.slice(2, 4), 16), parseInt(hex.slice(4, 6), 16), 255);
    if (hex.length === 8) return packRgba(parseInt(hex.slice(2, 4), 16), parseInt(hex.slice(4, 6), 16), parseInt(hex.slice(6, 8), 16), parseInt(hex.slice(0, 2), 16));
  }
  const known = namedColor(s);
...
function namedColor(s: string): string | undefined {
