...
export function fitPageMatrix(opts: FitOptions): Matrix2D {
...
export function matrixForW2d(page: W2dTextPageData, canvasWidth: number, canvasHeight: number, zoom = 1, panX = 0, panY = 0): Matrix2D {
  const b = page.bounds;
  const minX = b?.minX ?? 0;
  const minY = b?.minY ?? 0;
